...
export function httpRequest(url, callback)
{
    ...
    .then(function(content) { callback(content) })
}

export function httpRequestBody(url, callback, formData)
{
...
function addElement(parent, enfant, class1){
    let elmtEnfant = document.createElement(enfant);
    document.querySelector(parent).appendChild(elmtEnfant);
    elmtEnfant.classList.add(class1);
}
